...
def callback(recognizer, audio):
    try:
        ...
        print("[log] Я не понимаю что ты говоришь!")
    except sr.RequestError as e:
        print("[log] Я не понимаю что ты говоришь! Проверьте свой интернет")
...
def execute_cmd(cmd):
    if cmd == 'time':
        ...
        speak("настоящее время " + str(now.hour) + ":" + str(now.minute))

    elif cmd == 'radio':
        # Radioni yoqish
        # Bu yerga musiqa ijro etish kodini qo'shing
        pass

    elif cmd == 'stupid1':
        # Yemagan xazil
        speak("я не программист")
